// server/src/utils/bonusCalculator.ts
...
export function getApplicableBonuses(
  ...
  return bonuses.filter((bonus, idx) => {
    if (bonus.conditionType === 'day_of_week') {
      const match = bonus.daysOfWeek?.includes(dayOfWeek) ?? false;
      console.log('[server bonusCalculator] day_of_week check [bonus', idx, ']:', '| bonus days:', bonus.daysOfWeek, '| entry day:', dayOfWeek, '| match:', match);
      return match;
    }
    if (bonus.conditionType === 'time_range') {
      if (!bonus.startTime || !bonus.endTime) {
        console.log('[server bonusCalculator] time_range SKIP [bonus', idx, ']: missing start/end');
        return false;
      }
      const match = timeRangesOverlap(startTime, endTime, bonus.startTime, bonus.endTime);
      console.log('[server bonusCalculator] time_range check [bonus', idx, ']:', '| bonus:', bonus.startTime, '-', bonus.endTime, '| entry:', startTime, '-', endTime, '| match:', match);
      return match;
    }
    if (bonus.conditionType === 'specific_dates') {
      const match = bonus.specificDates?.includes(dateStr) ?? false;
      console.log('[server bonusCalculator] specific_dates check [bonus', idx, ']:', '| bonus dates:', bonus.specificDates, '| entry date:', dateStr, '| match:', match);
      return match;
    }
    return false;
  });
