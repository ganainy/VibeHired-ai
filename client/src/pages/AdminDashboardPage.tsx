...
const AdminDashboardPage: React.FC = () => {
 ...
 <p className="text-sm text-secondary-color italic">No provider calls recorded yet.</p>
 )}
 ...
 <span className="text-secondary-color">{item.count}</span>
 </div>
 ))}
 </div>
 </div>
 ...
 <span className="text-green">{stats.externalCalls.successful}</span>
 </div>
 <div className="flex items-center justify-between">
 <span className="font-medium">Failed</span>
 <span className="text-error">{stats.externalCalls.failed}</span>
 </div>
 <div className="flex items-center justify-between">
 ...
 className="w-full sm:max-w-sm px-3 py-2 text-sm rounded-lg border border-theme bg-surface text-primary-color placeholder:text-muted-color focus:outline-none focus:ring-2 focus:ring-gold/50"
 />
 </div>
 <TableOrCards
 ...
 { key: 'modelName', label: 'Model', render: (c: any) => c.modelName || '-' },
 { key: 'userEmail', label: 'User', render: (c: any) => c.userEmail || '-' },
 { key: 'creditUsed', label: 'Credit Used', render: (c: any) => formatCreditUsed(c) },
 {
 key: 'status',
 label: 'Status',
 render: (c: any) => (
 <span className={`px-2 py-1 rounded-full text-[10px] font-bold uppercase ${c.success ? 'bg-[var(--jade-bg)] text-[var(--jade)]' : 'bg-rose-100 text-rose-700'}`}>
 {c.statusCode || (c.success ? 'OK' : 'ERR')}
 </span>
 )
 },
 { key: 'durationMs', label: 'Latency', render: (c: any) => `${c.durationMs} ms` },
 { key: 'createdAt', label: 'Time', render: (c: any) => new Date(c.createdAt).toLocaleString() },
 ]}
 emptyMessage="No AI or Apify calls recorded yet."
 />
 {filteredExternalCalls.length > 0 && (
 <div className="mt-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
 <p className="text-xs md:text-sm text-secondary-color">
...
className="px-3 py-1.5 text-xs md:text-sm rounded-lg border border-theme text-secondary-color disabled:opacity-50 disabled:cursor-not-allowed hover:bg-[var(--bg-elevated)] transition-colors"
  >
  Previous
  </button>
  <span className="text-xs md:text-sm text-secondary-color min-w-[88px] text-center">
...
className="px-3 py-1.5 text-xs md:text-sm rounded-lg border border-theme text-secondary-color disabled:opacity-50 disabled:cursor-not-allowed hover:bg-[var(--bg-elevated)] transition-colors"
 ...
 <span className="text-secondary-color">{count} users ({((count / stats.totalUsers) * 100).toFixed(1)}%)</span>
 </div>
<div className="h-2 bg-elevated rounded-full overflow-hidden">
  <div
  className={`h-full rounded-full ${tier === 'free' ? 'bg-[var(--text-muted)]' :
 ...
 label: 'Status',
 render: (p: any) => (
 <span className={`px-2 py-1 rounded-full text-[10px] font-bold uppercase ${p.status === 'succeeded' ? 'bg-[var(--jade-bg)] text-[var(--jade)]' : 'bg-elevated text-secondary-color'}`}>
 {p.status}
 </span>
 )
 },
...
const StatCard: React.FC<{ label: string; value: string | number; icon: React.ReactNode; color: string }> = ({ label, value, icon, color }) => {
 ...
 emerald: 'bg-[var(--jade-bg)] text-green',
 ...
 <p className="text-secondary-color text-xs md:text-sm font-medium truncate">{label}</p>
