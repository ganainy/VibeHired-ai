...
const SortableProjectItem: React.FC<SortableProjectItemProps> = ({ project, onToggleVisibility }) => {
 ...
 ref={setNodeRef}
 style={style}
className="flex items-center justify-between p-4 border border-theme rounded-lg hover:bg-[var(--bg-elevated)] transition-colors cursor-move"
  >
...
<h4 className="font-medium text-primary-color">{project.title}</h4>
 ...
 <span
 key={idx}
 className="text-xs px-2 py-1 bg-[var(--bg-elevated)] text-secondary-color rounded"
 >
 {tech}
 </span>
 ))}
...
const PortfolioSetupPage: React.FC = () => {
 ...
 if (isLoading) {
 return (
 <div className="min-h-screen p-8 flex items-center justify-center" style={{ backgroundColor: 'var(--bg-base)' }}>
 <Spinner size="lg" />
 </div>
 );
 }
 ...
 {/* Page Header */}
 <div>
<h1 className="text-2xl font-bold" style={{ color: 'var(--text-primary)' }}>Your Portfolio</h1>
...
<div className="flex border-b border-theme gap-6">
  ...
  : 'border-transparent text-muted-color hover:text-secondary-color'
  }`}
  >
  Setup
  </button>
 ...
 */}
 ...
 <div>
<h2 className="text-base font-bold" style={{ color: 'var(--text-primary)' }}>Connect Your Accounts</h2>
  <p className="text-xs text-muted-color">We pull your projects and experience directly no copy-pasting.</p>
 </div>
 </div>

 <div className="grid grid-cols-1 md:grid-cols-2 gap-4" data-onboarding="primary-action">
...
<p className="font-semibold text-sm" style={{ color: 'var(--text-primary)' }}>GitHub</p>
 ...
 {isGitHubConnected && (
<span className="text-xs px-2 py-0.5 rounded-full bg-[var(--jade-bg)] text-green font-medium flex-shrink-0">Connected</span>
  )}
  </div>
  ...
  className="w-full px-3 py-2 text-sm border border-theme rounded-lg bg-white text-primary-color focus:outline-none focus:ring-2 focus:ring-primary"
  />
  <p className="text-xs text-muted-color mt-1">Only needed to avoid rate limits on public repos.</p>
  </div>
  </div>
 ...
 className="flex items-center justify-center gap-2 h-9 px-4 rounded-lg text-sm font-semibold text-white transition-colors disabled:opacity-40 disabled:cursor-not-allowed" style={{ backgroundColor: 'var(--text-primary)' }}
 >
 {isImporting ? (
...
? 'bg-[var(--ember-bg)] border-[var(--ember)] text-ember'
 ...
 <div>
<h2 className="text-base font-bold" style={{ color: 'var(--text-primary)' }}>Choose Projects to Display</h2>
  <p className="text-xs text-muted-color">Drag to reorder toggle the switch to show or hide each project.</p>
 </div>
 </div>
 {isGitHubConnected && (
 <button
 onClick={handleImportGitHub}
 disabled={!githubUsername || isImporting}
 className="flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-lg border border-theme bg-white text-secondary-color hover:bg-[var(--bg-elevated)] transition-colors disabled:opacity-40"
 >
...
<div className="bg-white border border-theme rounded-xl p-5">
  ...
  <div className="text-center py-6 text-muted-color">
 ...
 <div>
<h2 className="text-base font-bold" style={{ color: 'var(--text-primary)' }}>Edit Your Profile Info</h2>
  <p className="text-xs text-muted-color">Auto-filled from LinkedIn you can edit anything before publishing.</p>
 </div>
 </div>
 {isLinkedInConnected && (
 <button
...
<div className="bg-white border border-theme rounded-xl p-5 space-y-4">
  ...
  <div>
  <label className="block text-xs font-medium text-secondary-color mb-1">Full Name</label>
  <input type="text" value={linkedInData.name} onChange={(e) => setLinkedInData({ ...linkedInData, name: e.target.value })} placeholder="Your full name" className="w-full px-3 py-2 text-sm border border-theme rounded-lg bg-white text-primary-color focus:outline-none focus:ring-2 focus:ring-primary" />
  </div>
<div>
 <label className="block text-xs font-medium text-secondary-color mb-1">Professional Title</label>
 <input type="text" value={linkedInData.title} onChange={(e) => setLinkedInData({ ...linkedInData, title: e.target.value })} placeholder="e.g. Software Engineer" className="w-full px-3 py-2 text-sm border border-theme rounded-lg bg-white text-primary-color focus:outline-none focus:ring-2 focus:ring-primary" />
 </div>
 </div>
 <div>
 <label className="block text-xs font-medium text-secondary-color mb-1">Location</label>
 <input type="text" value={linkedInData.location} onChange={(e) => setLinkedInData({ ...linkedInData, location: e.target.value })} placeholder="e.g. Berlin, Germany" className="w-full px-3 py-2 text-sm border border-theme rounded-lg bg-white text-primary-color focus:outline-none focus:ring-2 focus:ring-primary" />
 </div>
 <div>
 <label className="block text-xs font-medium text-secondary-color mb-1">Bio / Summary</label>
 <textarea value={linkedInData.bio} onChange={(e) => setLinkedInData({ ...linkedInData, bio: e.target.value })} rows={4} placeholder="A short intro about yourself" className="w-full px-3 py-2 text-sm border border-theme rounded-lg bg-white text-primary-color focus:outline-none focus:ring-2 focus:ring-primary resize-none" />
 </div>
 </div>

 {/* Visibility toggles */}
<div className="bg-white border border-theme rounded-xl p-5">
  <p className="text-xs font-semibold text-muted-color uppercase tracking-wide mb-3">What to show on your portfolio</p>
  <div className="space-y-2">
  {[
  ...
  <label key={key} className="flex items-center justify-between py-2 px-3 rounded-lg cursor-pointer hover:bg-[var(--bg-elevated)] transition-colors">
 ...
 <div>
<h2 className="text-base font-bold" style={{ color: 'var(--text-primary)' }}>Go Live</h2>
  <p className="text-xs text-muted-color">Make your portfolio publicly accessible with a shareable URL.</p>
 </div>
 </div>

 <div className="bg-white border border-theme rounded-xl p-6 flex flex-col sm:flex-row sm:items-center gap-6">
 <div className="flex-1 min-w-0">
 <div className="flex items-center gap-2 mb-2">
<span className={`inline-flex items-center gap-1.5 text-xs font-semibold px-2.5 py-1 rounded-full ${profile?.isPublished ? 'bg-[var(--jade-bg)] text-green' : 'bg-[var(--bg-elevated)] text-muted-color'}`}>
   <span className={`w-1.5 h-1.5 rounded-full ${profile?.isPublished ? 'bg-green' : 'bg-muted-foreground'}`}></span>
 ...
 */}
 ...
 target="_blank"
 rel="noopener noreferrer"
 className="group bg-white border border-theme rounded-xl overflow-hidden hover:shadow-lg transition-all flex flex-col h-full"
 >
 <div className="h-28 bg-gradient-to-r from-blue-500 to-primary/80 relative">
 {prof.profileImageUrl ? (
 <img src={prof.profileImageUrl} alt={prof.name} className="absolute -bottom-7 left-5 w-14 h-14 rounded-full border-4 border-white object-cover" />
 ) : (
 <div className="absolute -bottom-7 left-5 w-14 h-14 rounded-full border-4 border-white flex items-center justify-center text-lg font-bold text-muted-color" style={{ background: 'var(--bg-elevated)' }}>
 {prof.name?.charAt(0) || prof.username.charAt(0)}
 </div>
 )}
 </div>
 <div className="p-5 pt-10 flex-grow flex flex-col">
 <h3 className="font-bold text-primary-color truncate">{prof.name || prof.username}</h3>
 <p className="text-xs text-primary mb-2">@{prof.username}</p>
{prof.title && <p className="text-sm text-secondary-color font-medium mb-2 line-clamp-1">{prof.title}</p>}
  {prof.bio && <p className="text-sm text-muted-color line-clamp-3 flex-grow">{prof.bio}</p>}
  <div className="mt-4 pt-3 border-t border-theme flex justify-between text-xs text-muted-color">
 ...
 <div className="col-span-full text-center py-16 text-muted-color">
