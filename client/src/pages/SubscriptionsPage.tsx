...
const PlanCard: React.FC<{
 ...
 <span className="bg-gold text-gold-950 text-[10px] font-black px-3 py-1 rounded-full tracking-wide uppercase">
 Most Popular
 </span>
 </div>
 )}
 ...
 <span className="bg-green text-white text-[10px] font-black px-3 py-1 rounded-full tracking-wide uppercase">
 Current
 </span>
 </div>
 )}
 ...
 <span className="text-sm text-secondary-color">/ {plan.period}</span>
 </div>
 )}
 ...
 <span className="text-green mt-0.5"><CheckIcon /></span>
 {feature}
 </li>
 ))}
 </ul>
 ...
 ${isCurrentPlan
 ? 'bg-elevated text-muted-color cursor-not-allowed'
 : plan.popular
 ? 'bg-gold hover:bg-gold text-gold-950'
...
const SubscriptionsPage: React.FC = () => {
 ...
 <p className="text-secondary-color mt-1">Manage your subscription and credit usage.</p>
 </div>
 {PAYMENTS_ENABLED && user?.plan && user.plan !== 'free' && (
 <button
 onClick={() => handleSelectPlan(user.plan!)}
 disabled={isProcessing}
 className="shrink-0 px-4 py-2 text-sm font-bold bg-surface border border-theme rounded-xl hover:bg-[var(--bg-elevated)] transition-colors disabled:opacity-50"
 >
 Manage Billing 
 </button>
 )}
 ...
 {!PAYMENTS_ENABLED && (
 <div className="flex items-center gap-3 p-4 bg-[var(--ember-bg)] border border-[var(--ember)] rounded-2xl">
 <svg className="w-5 h-5 shrink-0 text-ember" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}>
 ...
 {/* Success Banner */}
 {successBanner && (
 <div className="flex items-center gap-3 p-4 bg-[var(--jade-bg)] border border-emerald-200 rounded-2xl text-green">
 <svg className="w-5 h-5 shrink-0 text-green" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}>
 ...
 <button onClick={() => setSuccessBanner(false)} className="p-1 hover:bg-[var(--jade-bg)] rounded-lg transition-colors">
 ...
 <div>
 <p className="text-xs font-bold uppercase tracking-wider text-muted-color mb-1">Current Plan</p>
 ...
 <p className="text-[10px] text-muted-color uppercase tracking-wider font-bold">Remaining</p>
 </div>
 <div className="h-10 w-px bg-elevated" />
 <div>
 <p className="text-2xl font-black text-primary-color">{usageInfo.usage.creditsUsed}</p>
 <p className="text-[10px] text-muted-color uppercase tracking-wider font-bold">Used</p>
 </div>
 <div>
 <p className="text-2xl font-black text-primary-color">{usageInfo.usage.creditLimit}</p>
 <p className="text-[10px] text-muted-color uppercase tracking-wider font-bold">Total</p>
 </div>
 </div>
 )}
 </div>
 {!isLoadingUsage && usageInfo && (
 <div>
 <div className="flex justify-between text-xs text-muted-color mb-1.5">
 <span>Credit usage</span>
 <span>{usedPct}%</span>
 </div>
 <div className="h-2 bg-elevated rounded-full overflow-hidden">
 <div
 className={`h-full rounded-full transition-all ${usedPct >= 90 ? 'bg-red-500' : usedPct >= 70 ? 'bg-amber-500' : 'bg-green'}`}
 style={{ width: `${usedPct}%` }}
 />
 </div>
 </div>
 )}
 {isLoadingUsage && <div className="flex items-center gap-2 text-muted-color text-sm"><Spinner size="sm" /> Loading usage...</div>}
 </div>
 ...
 {/* Footer */}
 <p className="text-sm text-muted-color text-center pb-4">
 Questions?{' '}
 <button onClick={() => navigate('/settings')} className="text-secondary-color hover:underline font-medium">
 Contact support
 </button>
 </p>
...
const FAQItem: React.FC<{ question: string; answer: string }> = ({ question, answer }) => {
 ...
 <svg
 className={`w-4 h-4 text-muted-color transition-transform shrink-0 ml-4 ${isOpen ? 'rotate-180' : ''}`}
