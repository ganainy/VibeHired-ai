...
// Icon Components
const EyeIcon = () => (
<svg className="w-5 h-5 text-secondary-color" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}>
...
const ConfirmModal: React.FC<ConfirmModalProps> = ({
 ...
 <div className="rounded-lg shadow-warm-lg max-w-md w-full mx-4 p-6" style={{ backgroundColor: 'var(--bg-surface)' }}>
...
const SettingsPage: React.FC = () => {
 ...
 if (isLoading) {
 return (
 <div className="flex justify-center items-center min-h-screen" style={{ backgroundColor: 'var(--bg-base)' }}>
 <Spinner size="lg" />
 </div>
 );
 }
 ...
 return (
 <div className="min-h-screen py-8 sm:py-10 px-4" style={{ backgroundColor: 'var(--bg-base)', backgroundImage: "radial-gradient(circle at 0% 0%, color-mix(in srgb, var(--accent) 10%, transparent), transparent 36%)" }}>
 ...
 {/* Active Plan Card */}
 <div className="rounded-2xl shadow-warm-lg border border-theme p-6 h-fit" style={{ backgroundColor: 'var(--bg-surface)' }}>
 <div className="flex items-center justify-between mb-5">
 <h3 className="text-xs font-semibold text-secondary-color uppercase tracking-[0.16em]">Active Plan</h3>
 <span className={`px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-tight ${user?.plan === 'pro' || user?.plan === 'premium' ? '' : 'bg-elevated text-secondary-color'}`}
  style={user?.plan === 'pro' || user?.plan === 'premium' ? { backgroundColor: 'var(--accent-bg)', color: 'var(--accent)' } : undefined}
>
 {user?.plan || 'Free'}
 </span>
 </div>
...
<div className="rounded-xl border border-theme p-3 bg-elevated/70">
<p className="text-[11px] uppercase tracking-wide text-secondary-color">Remaining</p>
<p className="text-xl font-bold text-primary-color">{creditsRemaining}</p>
</div>
<div className="rounded-xl border border-theme p-3 bg-elevated/70">
<p className="text-[11px] uppercase tracking-wide text-secondary-color">Used</p>
<p className="text-xl font-bold text-primary-color">{creditsUsed}</p>
</div>
<div className="rounded-xl border border-theme p-3 bg-elevated/70">
<p className="text-[11px] uppercase tracking-wide text-secondary-color">Limit</p>
<p className="text-xl font-bold text-primary-color">{creditLimit}</p>
 ...
 <div className="w-full bg-elevated h-2.5 rounded-full mb-6 overflow-hidden">
<div
  className="h-full transition-all duration-700"
  style={{ width: `${creditsRemainingPct}%`, backgroundColor: 'var(--accent)' }}
/>
 </div>

 <div className="rounded-xl border border-theme bg-elevated/70 p-3 mb-5">
...
<div className="h-1.5 bg-elevated rounded-full overflow-hidden mb-2.5">
  <div className="h-full rounded-full transition-all duration-700" style={{ width: `${creditsUsedPct}%`, backgroundColor: 'var(--accent)' }} />
 </div>
 ...
 className="w-full py-2.5 rounded-lg border border-theme text-xs font-semibold text-secondary-color hover:bg-[var(--bg-elevated)] transition-colors flex items-center justify-center gap-2"
 >
 ...
 className="w-full py-2.5 rounded-lg text-gold-950 text-xs font-bold transition-all shadow-md shadow-gold/20"
  style={{ backgroundColor: 'var(--accent)' }}
 >
 Upgrade to Pro
 </button>
 ) : (
 <div className="w-full py-2.5 rounded-lg bg-elevated text-muted-color text-xs font-semibold text-center cursor-not-allowed">
 Paid plans coming soon
 </div>
 )}
 <Link
 to="/subscriptions"
 className="w-full mt-2 py-2 block text-center text-xs text-secondary-color hover:text-primary-color underline"
 >
 View all plans
 </Link>
 </>
...
<div className="rounded-xl shadow-warm-lg border border-theme overflow-hidden" style={{ backgroundColor: 'var(--bg-surface)' }}>
<div className="p-4 sm:p-6 border-b border-theme bg-elevated">
 ...
 <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium bg-elevated text-secondary-color">
 Not Connected
 </span>
 )}
 </div>
 ...
 className="flex items-center gap-1.5 px-3 py-1.5 text-sm font-medium rounded-lg border border-red-300 text-red-600 hover:bg-red-50 disabled:opacity-50 transition-colors"
 >
 ...
 className="flex items-center gap-2 px-4 py-2 text-sm font-medium rounded-lg border border-theme text-secondary-color hover:bg-[var(--bg-elevated)] disabled:opacity-50 transition-colors shadow-sm"
 >
 {isConnectingGoogle ? (
 <Spinner size="sm" />
 ) : (
 ...
 {/* Usage Breakdown */}
 <div className="xl:col-span-3 rounded-2xl shadow-warm-lg border border-theme p-6" style={{ backgroundColor: 'var(--bg-surface)' }}>
 ...
 <div className="flex items-center gap-2">
 <div className="px-3 py-1.5 rounded-full text-xs font-semibold bg-elevated text-primary-color">
 ...
 <div key={idx} className="h-8 rounded-lg bg-elevated animate-pulse" />
 ))}
 </div>
 ) : (
 ...
 return (
 <div key={item.label} className="rounded-xl border border-theme p-2.5">
 <div className="flex items-center justify-between text-xs mb-2">
<span className="text-secondary-color">
{item.label} <span className="text-muted-color"> {item.cost}</span>
 </span>
 <span className="font-semibold text-primary-color">{item.count}</span>
 </div>
<div className="h-1.5 bg-elevated rounded-full overflow-hidden">
  <div className="h-full rounded-full" style={{ width: `${widthPct}%`, backgroundColor: 'var(--accent)' }} />
 </div>
 </div>
 );
 })}
...
<div className="pt-4 mt-4 border-t border-theme flex justify-between text-xs">
<span className="text-muted-color italic">Resets on</span>
